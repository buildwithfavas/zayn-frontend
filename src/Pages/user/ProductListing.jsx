...
const ProductListing = () => {
  ...
  const totalProducts = data?.totalPosts || 0;
  ...
  useEffect(() => {
    ...
    if (catParams.category && catParams.subCategory && catParams.thirdCategory) {
      dispatch(
        setParams({
          category: [catParams.category],
          subCategory: catParams.subCategory,
      ...
      <div className="min-h-screen overflow-x-hidden" style={{ backgroundColor: "#e7e7e3" }}>
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-6 py-6">
          ...
          <div className="breadCrumbs mb-4">
            {catParams.category && catParams.subCategory && catParams.thirdCategory ? (
              <Breadcrumbs aria-label="breadcrumb">
                <Link to="/" className="link transition text-[14px]! hover:underline text-gray-700">
                  Home
                </Link>
                <Link
                  to={`/${catParams.category}`}
                  className="link transition text-[14px]! hover:underline text-gray-700"
                >
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 text-sm">
                Showing {startProduct}-{endProduct} out of {totalProducts} products
              </p>
            </div>
            <div className="relative hidden sm:block">
              <select
                value={params.sortBy || "default"}
                onChange={(e) => dispatch(setParams({ sortBy: e.target.value }))}
          ...
          <div className="flex gap-8">
            ...
            <div className="w-64 shrink-0 hidden lg:block">{renderFilters()}</div>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <div className="sm:hidden flex flex-wrap items-center justify-center gap-2 mt-2 w-full">
                    {/* Prev */}
                    <button
                      onClick={() => dispatch(setParams({ page: Math.max(1, currentPage - 1) }))}
                    ...
                    {visiblePagesMobile.map((page) => (
                      <button
                        key={`m-${page}`}
                        onClick={() => dispatch(setParams({ page }))}
                    ...
                    {visiblePagesMobile[visiblePagesMobile.length - 1] < totalPages && (
                      <>
                        ...
                        <button
                          onClick={() => dispatch(setParams({ page: totalPages }))}
                          className="w-8 h-8 text-xs rounded-lg border"
                          style={{
                            backgroundColor: "#e7e7e3",
                            color: "#232321",
                    ...
                    <button
                      onClick={() =>
                        dispatch(setParams({ page: Math.min(totalPages, currentPage + 1) }))
                      }
                    ...
                    {visiblePagesDesktop.map((page) => (
                      <button
                        ...
                        className={`w-10 h-10 text-sm font-medium rounded-lg border transition-colors ${
                          currentPage === page ? "bg-black text-white" : "hover:opacity-80"
                        }`}
                        style={
                          currentPage === page
                            ? { backgroundColor: "#232321", color: "white", borderColor: "#232321" }
        ...
        <div className="fixed inset-0 z-50 sm:hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-white rounded-t-2xl shadow-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setIsSortModalOpen(false)} aria-label="Close Sort">
                ✕
              </button>
            </div>
            <div className="space-y-2">
              {[
                ...
                >
                  ...
                  <input
                    type="radio"
                    name="sort"
                    checked={(params.sortBy || "default") === opt.value}
                    onChange={() => dispatch(setParams({ sortBy: opt.value }))}
                  />
                </label>
              ))}
            </div>
