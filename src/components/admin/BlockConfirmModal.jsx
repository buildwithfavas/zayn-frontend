 ...
 * @param {boolean} isBlocked - Current blocked state
 * @param {function} onConfirm - Callback when user confirms
 * @param {function} onCancel - Callback when user cancels
 */
const BlockConfirmModal = ({ open, isBlocked, onConfirm, onCancel }) => {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-sm p-5 z-10">
        <div className="text-base font-semibold text-gray-900">
          {isBlocked ? "Unblock" : "Block"} Category
        </div>
        <div className="mt-2 text-sm text-gray-600">
          Are you sure you want to {isBlocked ? "unblock" : "block"} this category?
        </div>
