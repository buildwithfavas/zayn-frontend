import paymentCards from "../../assets/payment-cards.png";

const Footer = () => {
  const quickLinks = [
    { name: "Products", href: "#" },
    { name: "Prices drop", href: "#" },
    { name: "New products", href: "#" },
    { name: "Best sales", href: "#" },
    { name: "Contact us", href: "#" },
  ];

  const companyLinks = [
    { name: "Delivery", href: "#" },
    { name: "Legal Notice", href: "#" },
    { name: "About us", href: "#" },
    { name: "Secure payment", href: "#" },
  ];

  return (
    <footer className="bg-gray-900 text-gray-300 pt-8 sm:pt-10 pb-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 grid-flow-row-dense gap-3 sm:gap-5 md:gap-8 mb-6 sm:mb-8">
          {/* Contact Us */}
          <div className="order-2 md:order-auto">
            <h4 className="font-semibold text-white mb-3 sm:mb-4 text-base sm:text-lg">Contact us</h4>
            <p className="text-xs sm:text-sm mb-2">Zayn Collection - Mega Super Store</p>
            <p className="text-xs sm:text-sm mb-2">507-Union Trade Centre France</p>
            <p className="text-xs sm:text-sm mb-3 sm:mb-4 break-all">sales@yourcompany.com</p>
            <p className="text-lg sm:text-xl font-bold text-white mb-3 sm:mb-4">(+91) 9876-543-210</p>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-2 text-xs sm:text-sm">
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
              <span>Online Chat</span>
              <span className="hidden sm:inline text-gray-500">|</span>
              <span>Get Expert Help</span>
            </div>
          </div>

          {/* Quick Links */}
          <div className="order-2 md:order-auto">
            <h4 className="font-semibold text-white mb-3 sm:mb-4 text-base sm:text-lg">Quick Links</h4>
            <ul className="grid grid-cols-2 md:grid-cols-1 gap-y-1 md:gap-y-2 gap-x-4 text-xs sm:text-sm">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <a href={link.href} className="hover:text-white">
                    {link.name}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Our Company */}
          <div className="order-2 md:order-auto">
            <h4 className="font-semibold text-white mb-3 sm:mb-4 text-base sm:text-lg">Our company</h4>
            <ul className="grid grid-cols-2 md:grid-cols-1 gap-y-1 md:gap-y-2 gap-x-4 text-xs sm:text-sm">
              {companyLinks.map((link) => (
                <li key={link.name}>
                  <a href={link.href} className="hover:text-white">
                    {link.name}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Subscribe to Newsletter */}
          <div className="order-1 md:order-auto">
            <h4 className="font-semibold text-white mb-3 sm:mb-4 text-base sm:text-lg">Subscribe to newsletter</h4>
            <p className="text-xs sm:text-sm mb-3 sm:mb-4">
              Subscribe to our latest newsletter to get news about special discounts.
            </p>
            <form className="space-y-2 sm:space-y-3">
              <input
                type="email"
                placeholder="Your Email Address"
                className="w-full px-3 py-2 rounded-md text-gray-900 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border border-white"
              />
              <button className="w-full bg-blue-600 hover:bg-blue-700 py-2 rounded-md text-white text-xs sm:text-sm font-medium transition">
                Subscribe
              </button>
              <label className="flex items-start text-[10px] sm:text-xs">
                <input type="checkbox" className="mr-2 mt-0.5 flex-shrink-0" />
                <span>I agree to the terms and conditions and the privacy policy</span>
              </label>
            </form>
          </div>
        </div>

        {/* Bottom Footer Bar */}
        <div className="border-t border-gray-700 pt-4 sm:pt-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4">
            {/* Social Media Icons */}
            <div className="flex items-center gap-2 sm:gap-3">
              <a
                href="#"
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-700 flex items-center justify-center hover:bg-gray-600 transition"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-700 flex items-center justify-center hover:bg-gray-600 transition"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-700 flex items-center justify-center hover:bg-gray-600 transition"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 18.341c-.147 1.154-1.159 1.936-2.308 1.802-1.15-.134-1.936-1.159-1.802-2.308l1.97-18.341c.147-1.154 1.159-1.936 2.308-1.802 1.15.134 1.936 1.159 1.802 2.308zM12 2.603c-5.384 0-9.75 4.366-9.75 9.75s4.366 9.75 9.75 9.75 9.75-4.366 9.75-9.75S17.384 2.603 12 2.603z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-700 flex items-center justify-center hover:bg-gray-600 transition"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
            </div>

            {/* Copyright */}
            <p className="text-xs sm:text-sm text-gray-400 text-center">© 2024 - Ecommerce Template</p>

            {/* Payment Methods */}
            <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2">
              <span className="text-[10px] sm:text-xs text-gray-400">We accept:</span>
              <img
                src={paymentCards}
                alt="Payment methods: Visa, Mastercard, American Express, PayPal"
                className="h-5 sm:h-6"
              />
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;

